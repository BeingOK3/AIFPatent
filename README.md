...
开发中：部署级 Embedding Provider、PostgreSQL 缓存、严格 Version 范围的 pgvector 精确召回、RRF/章节权重/多样性内核、追问数据层及独立七节点固定 Workflow 已具备，但默认关闭且尚未开放追问 API。尚未完成：七节点业务 Handler、首次报告/追问共享 HybridRetriever 运行时接入、回答引用门禁、SSE/UI、reranker、Citation 前端精细展开、专利族变体与法律状态增强、多租户/认证。这些边界记录在 `development/followup-rag/development-log.md`。
