...
## 2026-07-31 — scalable-clustering-and-patent-links

### 已完成

- 在可扩展态势规格中补充完整的混合聚类流程：多字段向量、Top-K 候选召回、技术问题/机制/对象/场景/关键词/IPC-CPC 加权重排、相似度图、层次微簇、代表样本和程序审计。
- 明确单个全文向量不作为最终聚类依据，公司和时间不参与技术相似度；初始权重必须通过同方向/不同方向标注对校准。
- 新增统一 `PatentLinkResolver` 规划：报告中的公开号由后端生成 Allowlist 内的安全超链接，并统一覆盖 HTML、Markdown、CSV、技术分类和精读列表。

### 验证

- 文档差异和 Markdown 基础格式检查通过；本次不修改运行时源码。

## 2026-07-31 — scalable-landscape-independent-spec

### 已完成

- 新增独立的可扩展专利态势分析规格，不依赖历史实现过程解释目标需求。
- 冻结“全量方向账本 → 跨公司统一技术分类 → 公司切片与时间指标 → 有界趋势候选与叙述”的目标结构。
- 明确模型与程序的事实所有权、单次上下文预算、Evidence Packet、最小任务恢复、追加迁移、Report 3.0 和 20/100/200/1000 合成验收矩阵。
- 将完整分析数据与报告展示上限分离，长尾和 Noise 保持可追溯，不再以不可逆“其他类”替代完整技术结构。

### 验证

- 文档目录索引已更新；本次仅修改开发规格和开发记录，不修改运行时源码。

## 2026-07-30 — landscape-large-company-category-consolidation

### 已完成

- 修复 100 条级别检索结果在 `ANALYZE_COMPANIES` 的轻量分类阶段，因跨批次产生超过 20 个技术类别而在中间 Draft schema 直接失败的问题。
- 保留最终“每家公司最多 20 个技术类别”的产品边界；超过上限时，新增受约束的类别合并 Agent，仅基于已验证的轻量类别摘要、关键词、专利号和证据 ID 合并相近方向。
- 合并后仍由程序重新生成类别 ID，并校验每件合格专利恰好归属一个类别、每件专利均有自有证据引用；模型不能借此扩展专利集合或制造证据。
- 若模型在压缩时漏分或重复归属专利，程序不再对同一不可靠输出盲目重试：自动退回确定性的 `19 个保留类别 + 1 个其余已识别方向`，从原始已验证类别继承专利与证据成员。
- 合并输入不包含全文或精读结果，且类别摘要截断为轻量上下文，避免全量趋势分析意外演变为逐篇全文精读。

### 验证

- 新增 21 个跨批次独立类别的回归：验证先进入合并 Agent，输出压缩为 20 类且保持全量专利覆盖与证据可追溯；另覆盖模型压缩漏掉成员时的确定性回退。
...
- 将“重点友商”语义明确为“本次要检索的目标公司”，保留旧 API 字段以兼容已有 Run。
- 别名模型输出改用 Draft schema；程序按用户提交的公司逐项对齐，不再要求模型回显的 `primary_name` 必须逐字等于输入。
- Draft 接收层容忍模型附带未知字段、空别名或单字符串别名，再由程序统一裁剪为最多 12 个有效别名。
- 单公司输入时可把模型回显的中英文常用名绑定回用户主名称；多公司输出缺行或重复行时只对对应公司回退主名称，不丢弃其他公司的有效别名。
- 别名仍只用于扩大检索召回，正式申请人归属继续使用用户输入公司和已校验别名。
- 前端补充“重点友商（检索公司）”说明，避免把检索目标误解为待发现的竞争对手。

### 验证

- Ark `kimi-k2.6` 真实调用覆盖英伟达、NVIDIA、华为、Huawei、台积电、三星、英特尔、中科曙光、浪潮、联想等多组输入，别名和查询计划均成功生成。
- Landscape 全量测试 168 项通过。
- Python compileall、前端契约和 `git diff --check` 通过。
...
## 2026-07-29 — landscape-complete-set-batching

### 变更

- 抓取和逐件精读改为按 `analysis_limit` 作为批大小分段调度。
- 批处理只限制事件循环中同时创建的任务数量，不改变合格集合成员；所有候选仍会被尝试并逐件记录失败。
- 为旧版分析适配器保留无 `batch_size` 参数的兼容回退。
...
## 2026-07-29 — landscape-batch-adapter-compatibility

### 修复

- 为抓取批处理增加旧版测试替身/下游适配器兼容回退。
- 新批处理参数不再破坏仍使用旧 `_fetch_documents(run_id, hits)` 签名的实现。
## 2026-07-29 — landscape-company-alias-filter-fix

### 修复

- 过滤和公司归属改用已校验的有效检索 Scope，使模型识别出的跨语言权利人名称可归入用户输入的公司。
- 用户只输入“英伟达”时，`NVIDIA Corporation` 等检索命中不再因语言差异全部进入 `COMPETITOR_NOT_CONFIRMED`。
- 用户界面仍只要求公司名或技术方向；中英文检索词由程序内部构造，时间窗口作为统一硬约束。

### 验证

- 新增英伟达/NVIDIA 跨语言权利人过滤回归测试。
## 2026-07-29 — landscape-target-architecture-aligned

### 已确认目标

- 公司技术趋势基于全部去重且合格专利的轻量技术指纹，不再要求每件专利先完成深度精读。
- 深度精读改为重点专利的可选增强流程，失败不阻塞全量公司趋势。
- 重点专利由程序按相关性、检索共识、方向代表性、时间代表性、同族布局和近期性选择。
- 当前只有一个 Provider，`provider_coverage` 按满分/中性项处理，不作为专利之间的区分性加分。
- 后续新增 Provider 后再启用跨 Provider 共识评分。

### 开发约束

- 后续 Coding Agent 必须区分 `U`（全部合格）、`L`（轻量方向指纹）、`A`（深度精读）；
...
## 2026-07-29 — landscape-company-trend-lightweight-input

### 实现

- 公司分析阶段优先消费每个合格候选的轻量技术指纹；
- 轻量分类结果覆盖全部可构造指纹的公司专利，不再依赖逐件深度分析结果；
- 跨公司趋势上下文支持轻量指纹、标题、关键词、时间桶和方向证据；
- 保留旧深度分析路径作为无轻量指纹时的兼容回退；
- 轻量公司任务完成结果持久化指纹，进程恢复后可从主阶段结果重建。

### 验证

- 轻量指纹、公司分类和趋势上下文测试通过；
- Landscape 全量回归：`163` 项通过。

## 2026-07-29 — landscape-deterministic-deep-selection

### 实现

- 新增重点专利确定性选择器；
- 先满足公司、技术方向和时间桶覆盖，再按综合分补足名额；
- 当前单 Provider 的 Provider 覆盖按中性满分处理，不产生跨专利差异；
- 选择结果包含分数、排名、时间桶和可审计理由码。
- 公司阶段输出会持久化本次 Run 的重点专利候选，供后续可选深度任务消费。
...
## 2026-07-29 — landscape-defer-deep-analysis-from-main

### 实现

- 主 Graph 的 `ANALYZE_PATENTS` 现在只记录“深度精读延后”，不再默认对全部合格专利逐件调用大模型；
- 公司分类和跨公司趋势先基于全量轻量指纹完成；
- 原 `analyze_patents()` 保留，供重点专利深度任务和兼容调用；
- 报告可处理深度精读尚未执行的 Run，并明确失败/延后状态。
