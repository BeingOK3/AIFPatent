...
- 技术方向模式把原始/中文词和英文扩展词合并为一条 `OR` 查询；原始输入不能被翻译结果替换；
- 友商模式为每个用户输入友商生成一条包含主名称和中英文别名的独立查询，任何友商不得因为总查询上限或前序友商别名过多而被跳过；
- 联合模式为每个友商生成一条“中英文技术词组 × 该友商名称组”的查询；不把所有公司压进一条全局 Top-N 查询；
...
公司数量分布使用严格过滤、跨查询去重后的全部唯一合格专利计算，不使用会重复的原始命中，也不使用已经受 `candidate_limit` 截断的候选子集。候选统计与报告规则见 `selection-report-v2-design.md`；中英文合并查询和当前精读加权规则见 `query-selection-v3-design.md`。
