...
def _docker_command(action: str) -> list[str]:
    ...
    commands = {
        ...
        "migrate": [
            ...
            'psql -v ON_ERROR_STOP=1 -U "$POSTGRES_USER" -d "$POSTGRES_DB" '
            "-f /docker-entrypoint-initdb.d/088_landscape_v4_query_plans.sql && "
            'psql -v ON_ERROR_STOP=1 -U "$POSTGRES_USER" -d "$POSTGRES_DB" '
            "-f /docker-entrypoint-initdb.d/089_landscape_v4_search_pages.sql && "
            'psql -v ON_ERROR_STOP=1 -U "$POSTGRES_USER" -d "$POSTGRES_DB" '
            "-f /docker-entrypoint-initdb.d/090_landscape_v4_publications.sql && "
            'psql -v ON_ERROR_STOP=1 -U "$POSTGRES_USER" -d "$POSTGRES_DB" '
            "-f /docker-entrypoint-initdb.d/091_landscape_v4_scale_gate.sql && "
            'psql -v ON_ERROR_STOP=1 -U "$POSTGRES_USER" -d "$POSTGRES_DB" '
            "-f /docker-entrypoint-initdb.d/092_landscape_v4_abstract_evidence.sql && "
            'psql -v ON_ERROR_STOP=1 -U "$POSTGRES_USER" -d "$POSTGRES_DB" '
            "-f /docker-entrypoint-initdb.d/093_landscape_v4_tasks.sql && "
            'psql -v ON_ERROR_STOP=1 -U "$POSTGRES_USER" -d "$POSTGRES_DB" '
            "-f /docker-entrypoint-initdb.d/094_landscape_v4_direction_classification.sql && "
            'psql -v ON_ERROR_STOP=1 -U "$POSTGRES_USER" -d "$POSTGRES_DB" '
            "-f /docker-entrypoint-initdb.d/095_landscape_v4_others.sql && "
            'psql -v ON_ERROR_STOP=1 -U "$POSTGRES_USER" -d "$POSTGRES_DB" '
            "-f /docker-entrypoint-initdb.d/096_landscape_v4_metrics_trends.sql && "
            'psql -v ON_ERROR_STOP=1 -U "$POSTGRES_USER" -d "$POSTGRES_DB" '
            "-f /docker-entrypoint-initdb.d/097_landscape_v4_workflow_state.sql && "
            'psql -v ON_ERROR_STOP=1 -U "$POSTGRES_USER" -d "$POSTGRES_DB" '
            "-f /docker-entrypoint-initdb.d/098_landscape_v4_analysis_units.sql && "
            'psql -v ON_ERROR_STOP=1 -U "$POSTGRES_USER" -d "$POSTGRES_DB" '
            "-f /docker-entrypoint-initdb.d/099_landscape_v4_organizations.sql && "
            'psql -v ON_ERROR_STOP=1 -U "$POSTGRES_USER" -d "$POSTGRES_DB" '
            "-f /docker-entrypoint-initdb.d/100_landscape_v4_patent_snapshots.sql && "
            'psql -v ON_ERROR_STOP=1 -U "$POSTGRES_USER" -d "$POSTGRES_DB" '
            "-f /docker-entrypoint-initdb.d/101_landscape_v4_reports.sql && "
            'psql -v ON_ERROR_STOP=1 -U "$POSTGRES_USER" -d "$POSTGRES_DB" '
            "-f /docker-entrypoint-initdb.d/102_landscape_v4_publication_set_date_excluded.sql && "
            'psql -v ON_ERROR_STOP=1 -U "$POSTGRES_USER" -d "$POSTGRES_DB" '
            "-f /docker-entrypoint-initdb.d/103_landscape_search_pages_max_pages.sql",
        ],
        "ensure-bucket": [
            "exec",
            "--no-TTY",
