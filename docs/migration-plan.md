...
1. 建立独立源码基线和提交规范。
2. 移除 OpenCode Runtime、旧 API、健康检查、安装逻辑和凭证回退。
3. 分离领域、基础设施和编排边界。
4. 建立 Typed Graph State、固定拓扑、重试、超时和事件桥接。
5. 迁移输入、规划、检索、文献分析、结论、价值、审计和报告节点。
6. 接入 LangChain BYOK 模型工厂并保留厂商兼容策略。
7. 保持 Case/Run API、SSE、调试日志和前端兼容。
