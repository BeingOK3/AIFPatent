(() => {
  ...
  function renderReport(report) {
    ...
    const coverageAuditView = coverageAudit ? `<p><b>${escapeHtml(coverageAudit.decision || "UNKNOWN")}</b> · 覆盖率 ${Math.round(Number(coverageAudit.coverage_ratio || 0) * 10000) / 100}% · 修复轮次 ${Number(coverageAudit.repair_round || 0)}</p>${(coverageAudit.limitations || []).map((message) => `<p class="muted">${escapeHtml(message)}</p>`).join("")}` : `<p class="muted">历史报告未记录公司趋势覆盖审计。</p>`;
