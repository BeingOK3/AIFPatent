(() => {
  ...
  async function loadDebug(runId, force = false) {
    ...
    try {
      ...
      const ranking = (debug.candidate_ranking || []).slice(0, 30).map((item) => `<div class="debug-row"><b>${escapeHtml(item.publication_number)} · ${escapeHtml(item.company)}</b><span>综合分 ${Number(item.score || 0).toFixed(3)} · 同族法域 ${Number(item.family_footprint || 0)} · ${item.selected ? "入选候选" : "未入选"}</span><em>${escapeHtml((item.reasons || []).join("；"))}</em></div>`).join("") || `<p class="muted">暂无候选排序。</p>`;
      $("debug-content").innerHTML = `<div class="debug-grid"><section><h4>步骤</h4>${steps}</section><section><h4>中英文技术词</h4>${direction}</section><section><h4>检索式</h4><ul class="debug-queries">${queries}</ul></section><section><h4>友商别名</h4>${aliases}${debug.alias_resolution_error ? `<p class="debug-warning">别名解析失败，已回退主名称：${escapeHtml(debug.alias_resolution_error)}</p>` : ""}</section><section><h4>Provider 明细</h4><div class="debug-chips">${provider}</div>${attempts}</section><section><h4>过滤 / 日期补全</h4><p class="debug-summary">原始命中 ${coverage.raw_hit_count || 0} · 合格 ${coverage.eligible_hit_count || 0} · 唯一候选 ${coverage.unique_candidate_count || 0} · 入选 ${coverage.selected_count || 0} · 截断 ${coverage.truncated_count || 0}</p><p class="debug-summary">排除：${excluded}</p><p class="debug-summary">缺日期命中 ${enrichment.missing_hit_count || 0} · 去重后 ${enrichment.unique_publication_count || 0} · 实际补全 ${enrichment.attempted_count || 0} · 复用 ${enrichment.reused_hit_count || 0}</p></section><section><h4>精读选样</h4><p class="debug-summary">目标 ${selection.target_count || 0} · 公司 ${selection.company_count || 0} · 公司覆盖 ${selection.company_coverage_complete === false ? "受精读上限限制" : "完整"} · 补位 ${selection.backfilled_count || 0}</p><p class="debug-summary">首选：${escapeHtml((selection.selected_publications || []).join("、") || "暂无")}</p><p class="debug-summary">实际尝试：${escapeHtml((selection.attempted_publications || []).join("、") || "暂无")}</p></section><section><h4>候选排序（前 30）</h4>${ranking}</section></div>`;
  ...
  function renderReport(report) {
    ...
    }).join("") || `<p class="muted">暂无聚类（可能是无成功精读文献）。</p>`;
    ...
    $("report-view").innerHTML = `<div class="metric-grid"><div class="metric"><b>${summary.candidate_count || 0}</b><span>唯一合格专利</span></div><div class="metric"><b>${summary.analyzed_count || 0}</b><span>成功精读</span></div><div class="metric"><b>${summary.cluster_count || 0}</b><span>技术聚类</span></div><div class="metric"><b>${summary.failed_analysis_count || 0}</b><span>精读失败</span></div></div><section class="report-section"><h3>本次检索到的友商别名</h3>${aliases}</section><section class="report-section"><h3>各公司专利数量</h3><p class="muted">统计口径：时间与友商条件过滤后、跨检索式去重的全部唯一专利。</p>${companyBars}</section><section class="report-section"><h3>公开法域布局</h3><div class="jurisdiction-list">${jurisdictions}</div></section><div class="report-section"><h3>技术聚类</h3>${clusters}</div><div class="report-section"><h3>逐件精读</h3>${patents}</div><div class="report-section"><h3>限制与失败</h3>${limitations}</div><div class="report-links"><a href="/api/landscape/runs/${encodeURIComponent(report.run_id)}/report.md">下载 Markdown</a><a href="/api/landscape/runs/${encodeURIComponent(report.run_id)}/patents.csv">下载 CSV</a></div>`;
  }
