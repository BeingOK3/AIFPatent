(() => {
  ...
  function renderReport(report) {
    ...
    $("report-view").innerHTML = `<div class="metric-grid"><div class="metric"><b>${summary.family_count ?? summary.candidate_count ?? 0}</b><span>唯一合格专利族</span></div><div class="metric"><b>${summary.publication_count ?? summary.candidate_count ?? 0}</b><span>合格公开文本</span></div><div class="metric"><b>${summary.analyzed_count || 0}</b><span>成功精读专利族</span></div><div class="metric"><b>${summary.company_profile_count || 0}</b><span>公司技术画像</span></div><div class="metric"><b>${summary.trend_count || 0}</b><span>跨公司趋势</span></div><div class="metric"><b>${summary.failed_analysis_count || 0}</b><span>精读失败</span></div></div><section class="report-section"><h3>本次检索到的友商别名</h3>${aliases}</section><section class="report-section"><h3>各公司专利族数量</h3><p class="muted">统计口径：时间与友商条件过滤后，优先按可靠 Family ID、其次按申请号聚合；身份缺失时按公开文本保守分开，不用标题相似度猜测合并。</p>${companyBars}</section><section class="report-section"><h3>公司技术画像</h3>${companyProfiles}</section><section class="report-section"><h3>跨公司整体技术趋势</h3>${companyTrends}</section><section class="report-section"><h3>公开法域布局</h3><div class="jurisdiction-list">${jurisdictions}</div></section><div class="report-section"><h3>技术聚类</h3>${clusters}</div><div class="report-section"><h3>逐族精读</h3>${patents}</div><div class="report-section"><h3>限制与失败</h3>${limitations}</div><div class="report-links"><a href="/api/landscape/runs/${encodeURIComponent(report.run_id)}/report.md">下载 Markdown</a><a href="/api/landscape/runs/${encodeURIComponent(report.run_id)}/patents.csv">下载 CSV</a></div>`;
  }
